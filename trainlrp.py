...
from torchFewShot.models.net import ModelwithLRP
...
args.gpu_devices = '0'  # 0 :3 2:1 1:0 3:2
args.nExemplars = 1
def main():
    args.save_dir = f'./result/{args.dataset}/CAM/{args.nExemplars}-shot-seed1-resnet12-lrpscore-proto-test-224-aug/'
...
def train(epoch, model, criterion, optimizer, trainloader, learning_rate, use_gpu):
    ...
    for batch_idx, (images_train, labels_train, images_test, labels_test, pids) in enumerate(trainloader):
        ...
        loss.backward()
        optimizer.step()
        l1.update(loss1.item(), pids.size(0))
        l2.update(loss2.item(), pids.size(0))
        losses.update(loss.item(), pids.size(0))
        batch_time.update(time.time() - end)
        end = time.time()
    print('total', l1.avg, l2.avg)
    print('Epoch{0} '
          'lr: {1} '
